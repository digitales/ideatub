@php
    ...
    $iconSizeClass = $compact ? 'w-8 h-8 rounded-lg' : 'w-10 h-10 rounded-xl';
@endphp
<span class="flex-shrink-0 {{ $iconSizeClass }} flex items-center justify-center {{ $iconWrapClass }}" aria-hidden="true">
    @if ($card->kind === 'draft')
        <svg @class(['w-3.5 h-3.5' => $compact, 'w-4 h-4' => ! $compact]) fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Z"/></svg>
    @elseif ($card->kind === 'inbox')
        <svg @class(['w-3.5 h-3.5' => $compact, 'w-4 h-4' => ! $compact]) fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75"/></svg>
    @elseif ($card->kind === 'revisit')
        <svg @class(['w-3.5 h-3.5' => $compact, 'w-4 h-4' => ! $compact]) fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/></svg>
    @else
...
<div class="min-w-0 flex-1">
    ...
    <p class="mt-0.5 text-sm font-medium text-deep-indigo leading-snug {{ $compact ? 'line-clamp-1' : 'line-clamp-2' }} group-hover:text-memory-violet transition-colors">{{ $card->title }}</p>
    @if ($card->subtitle && ! $compact)
        <p class="mt-0.5 text-[11px] text-slate-brand/60">{{ $card->subtitle }}</p>
    @endif
</div>
@if ($card->draftId === null)
    <svg class="flex-shrink-0 w-3.5 h-3.5 text-slate-brand/30 group-hover:text-memory-violet/60 transition-colors" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5"/></svg>
@endif
