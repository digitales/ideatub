...
@section('content')
    ...
    @if ($betaCodeRequired ?? false)
        <div class="mb-4">
            <label for="beta_code" class="block text-xs font-medium text-slate-brand mb-1">Beta access code</label>
            <input id="beta_code" name="beta_code" type="text" autocomplete="off" required
                   value="{{ old('beta_code') }}"
                   placeholder="Enter your beta access code"
                   form="register-form oauth-google-start oauth-github-start"
    ...
    <div class="grid grid-cols-2 gap-3">
        <form id="oauth-google-start" method="POST" action="{{ route('auth.google.start') }}">
            @csrf
            <button type="submit"
                    class="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-lg border border-memory-violet/15 bg-white/60 text-xs font-medium text-slate-brand hover:bg-white hover:border-memory-violet/30 transition">
        ...
        <form id="oauth-github-start" method="POST" action="{{ route('auth.github.start') }}">
            @csrf
            <button type="submit"
                    class="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-lg border border-memory-violet/15 bg-white/60 text-xs font-medium text-slate-brand hover:bg-white hover:border-memory-violet/30 transition">
