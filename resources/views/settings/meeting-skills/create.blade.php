...
@section('title', 'New meeting skill — IdeaTub')

@section('content')
<div class="max-w-[600px] mx-auto px-6 pt-16 pb-24">
    <h1 class="text-[28px] font-semibold text-deep-indigo leading-snug mb-2">New meeting skill</h1>
    <p class="text-sm text-slate-brand mb-8">Meeting brief workflow only in this version.</p>
    ...
    <div class="rounded-2xl border border-memory-violet/20 bg-white/80 backdrop-blur p-6 shadow-[0_4px_24px_rgba(109,106,247,0.08)]">
        <form method="POST" action="{{ route('settings.skills.meeting.store') }}">
            @csrf
            @include('settings.meeting-skills._form', ['skill' => null, 'latest' => null])
