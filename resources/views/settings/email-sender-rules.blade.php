@php
    ...
    $filtersActive = ($filterAction ?? null) !== null || ($filterQ ?? '') !== '';
@endphp
...
<div class="max-w-[720px] mx-auto px-6 pt-16 pb-24">
            ...
            <div>
                ...
                >
                    ...
                    @foreach ($actionLabels as $value => $label)
                        <option value="{{ $value }}" @selected(($filterAction ?? null) === $value)>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
            <button
    ...
    <div class="rounded-2xl border border-memory-violet/20 bg-white/80 backdrop-blur p-6 shadow-[0_4px_24px_rgba(109,106,247,0.08)] mb-6">
        ...
        @if ($rules->total() === 0)
            @if ($filtersActive)
                <p class="text-sm text-slate-brand">No rules match these filters.</p>
            @else
                <p class="text-sm text-slate-brand">No sender rules yet. Add one below.</p>
            @endif
        @else
            @if ($filtersActive)
                <p class="text-sm text-slate-brand mb-4">{{ $rules->total() }} matching rules</p>
            @endif
            <ul class="space-y-6">
