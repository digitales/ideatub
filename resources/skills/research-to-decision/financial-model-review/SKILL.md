...
description: |
 ...
 decision the model supports. Optional IdeaTub MCP search and capture can pull prior
...
<!--
  IdeaTub distribution — adapted from OB1 financial-model-review.
  Upstream: https://github.com/NateBJones-Projects/OB1/tree/main/skills/financial-model-review
-->
...
**Tools:** `search_thoughts`, `browse_recent`, `capture_plan`, `capture_thought`. MCP cannot read local files; paste **`content`** into `capture_plan`. Use one **`plan_slug`** per workflow run and set **`project`**. See repository file `resources/prompts/research-to-decision-ideatub.md`.
...
- prior versions, management guidance, or historical notes from IdeaTub (**`search_thoughts`**)
...
7. Optionally use IdeaTub MCP.
 - Run **`search_thoughts`** for prior model assumptions, earlier reviews, or management claims.
 - Capture the final review memo or most important flags with **`capture_plan`** (`doc_type`: usually `research`; reuse **`plan_slug`**) after completion.
