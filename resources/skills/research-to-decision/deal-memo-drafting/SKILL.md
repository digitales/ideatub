<!--
  IdeaTub distribution — adapted from OB1 deal-memo-drafting.
  Upstream: https://github.com/NateBJones-Projects/OB1/tree/main/skills/deal-memo-drafting
-->
---
...
description: |
 ...
 outputs, and supporting documents. Optional IdeaTub MCP search and capture can pull
...
**Tools:** `search_thoughts`, `browse_recent`, `capture_plan`, `capture_thought`. For a durable memo use **`capture_plan`** with `doc_type`: `decision` or `research` (your choice for Stream). MCP cannot read local files; paste **`content`**. See repository file `resources/prompts/research-to-decision-ideatub.md`.
...
6. Optionally use IdeaTub MCP.
 - Run **`search_thoughts`** for prior deal notes, past meetings, or earlier thesis fragments.
 - Capture the final memo summary or recommendation with **`capture_plan`** after the draft is complete.
