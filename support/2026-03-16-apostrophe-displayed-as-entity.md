...
- **Double-escaping in Blade (confirmed when DB had plain text):** Laravel’s `{{ }}` already escapes (compiles to `e()`). Using `{{ e($thought->getDecodedContent()) }}` escaped twice: (1) inner `e("Daphne's")` → `Daphne&#039;s`; (2) Blade’s `{{ }}` escaped that again → `Daphne&amp;#039;s` in HTML, so the browser showed the literal `Daphne&#039;s`. Fix: use `{{ $thought->content }}` only so Blade escapes once.
