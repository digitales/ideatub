...
final class ThoughtDetailPresenter
{
    ...
    public function videoTranscriptText(): ?string
    {
        ...
        $transcriptChild = $this->thought->comments->first(
            fn (Thought $comment): bool => data_get($comment->metadata, 'video_section_type') === 'transcript'
        );
        ...
        try {
            $this->resolvedVideoTranscriptText = $this->demoText($raw, 'video_transcript_text') ?? 'Demo content hidden';
        } catch (\Throwable $e) {
            $this->logDemoObfuscationFailure(
    ...
    public function videoLatestResearchUrl(): ?string
    {
        ...
        if ($research === null || (int) $research->user_id !== (int) $this->thought->user_id) {
            $this->resolvedVideoLatestResearchUrl = null;
        ...
        $this->resolvedVideoLatestResearchUrl = route('idea.research.show', $research);
