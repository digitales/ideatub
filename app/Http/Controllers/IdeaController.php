...
class IdeaController extends Controller
{
    ...
    public function index(Request $request): View|RedirectResponse|JsonResponse
    {
        ...
        $indexThoughtCollection =
            $thoughts instanceof LengthAwarePaginator
                ? $thoughts->getCollection()
                : collect($thoughts);
        $newsletterResearchStatusPresenters = $this->buildEmailNewsletterResearchStatusPresenters(
            $indexThoughtCollection
    ...
    public function show(Thought $thought): View|RedirectResponse
    {
        ...
        $senderRuleContext = $emailDetailPreloadedImport
            ? $this->thoughtEmailSenderContextResolver->resolve(
                $thought,
                $importedEmail,
                usePreloadedImportedEmail: true
            )
            : null;
        $contentHtml = null;
        ...
        if ($thought->source !== 'email') {
            ...
            $sectionThoughts = $thought->childThoughts
                ->filter(
                    fn (
                        Thought $comment
                    ): bool => $this->isStructuredDocumentSection($comment)
                )
                ->sortBy(
            ...
            $documentSectionHtmlChunks = $sectionThoughts
                ->map(
                    fn (
                        Thought $section
                    ): string => $this->renderedThoughtBodyHtml($section)
                )
                ->all();
        ...
        $emailResearchPreview = $emailDetailPreloadedImport
            ? $this->buildEmailResearchPreview(
                $thought,
                $importedEmail,
                usePreloadedImportedEmail: true
            )
        ...
        $newsletterResearchStatus = $emailDetailPreloadedImport
            ? NewsletterResearchStatusPresenter::fromArray(
                $this->demoSafeNewsletterResearchStatusPayload(
                    $this->buildEmailNewsletterResearchStatus(
                        $thought,
                        $importedEmail,
                        usePreloadedImportedEmail: true
                    )
        ...
        if (app(DemoMode::class)->enabled()) {
            ...
            $thoughtOutgoingLinks = $thoughtOutgoingLinks->map(function (
                ThoughtLink $link
            ...
            $thoughtIncomingLinks = $thoughtIncomingLinks->map(function (
                ThoughtLink $link
            ) use ($obfuscator): ThoughtLink {
    ...
    public function stream(Request $request): View|JsonResponse|RedirectResponse
    {
        ...
        $streamNewsletterPresenters = $this->buildEmailNewsletterResearchStatusPresenters(
            $thoughts->getCollection()
        );
    ...
    private function streamCollectionResponse(
        ...
        $typedStreamNewsletterPresenters = $this->buildEmailNewsletterResearchStatusPresenters(
            $thoughts->getCollection()
        );
    ...
    private function buildIdeaIndexCardPresenters(
        ...
        $videoResearchUrlByThoughtId = $this->buildVideoResearchUrlByThoughtId(
            $collection
        );

        $replyableIndex = $replyableIndexStart;

        return $collection->map(function (Thought $thought) use (
            &$replyableIndex,
     ...
     */
    private function buildVideoResearchUrlByThoughtId(
        Collection $thoughts
    ): array {
        ...
        return $researchIdsByThoughtId
            ->mapWithKeys(function (string $researchId, string $thoughtId) use (
                $resolvedUrlsByResearchId
            ): array {
    ...
    private function firstPageCreatedAtCursor(
        LengthAwarePaginator $thoughts,
        bool $ascending
    ): ?string {
        if ($thoughts->isEmpty()) {
    ...
    public function ideas(Request $request): View|JsonResponse
    {
        ...
        $user = $request->user();
        $manualResearchSkills =
            $user instanceof User
                ? $this->manualResearchSkillsForUser($user)
                : collect();
        $hasEligibleDefaultSkill = $manualResearchSkills->contains(
            fn (ResearchSkill $skill): bool => $skill->is_default &&
                $skill->allow_auto_run &&
                (bool) $skill->latest_version_exists
        );
        $autoRunEnabled =
            $user instanceof User && $hasEligibleDefaultSkill
                ? (bool) UserPreference::get(
                    $user,
    ...
    private function buildIdeaListItemPresenters(
        ...
        return $ideas
            ->getCollection()
            ->map(function (Thought $thought) use ($researchByIdea) {
                $researchList = $researchByIdea->get($thought->id, collect());
                $status = IdeaResearchStatusPresenter::from(
                    $thought,
                    null,
                    null
                );

                return IdeaListItemPresenter::from(
                    $thought,
                    $researchList,
                    $status
                );
     ...
     */
    private function buildCompletedIdeaPresenters(
        LengthAwarePaginator $ideas
    ): Collection {
        return $ideas
            ->getCollection()
            ->map(
                fn (Thought $thought) => CompletedIdeaPresenter::from($thought)
            )
            ->values();
    }
    ...
    public function showResearch(Request $request, Thought $thought, ?string $page = null): View|RedirectResponse
    {
        ...
        $editorialLinkSummaries = $this->buildResearchEditorialLinkSummaryViewModel(
            $thought
        );
     ...
     */
    private function resolveLinkedVideoForResearchThought(
     ...
     * }
     */
    private function buildResearchEditorialLinkSummaryViewModel(
        Thought $root
    ): array {
        ...
        $sections = $groups
            ->map(function (Collection $items, string $labelKey) {
                ...
                $sortedItems = $items
                    ->sort(
                        fn (
                            ThoughtLinkSummary $a,
                            ThoughtLinkSummary $b
                        ) => $this->compareEditorialLinkSummariesForDisplay(
                            $a,
                            $b
                        )
                    )
                ...
                $mappedItems = $sortedItems
                    ->map(
                        ...
                        ) => $this->mapEditorialLinkSummaryRowForResearchView(
                            $row
                        )
                    )
                    ->all();
    ...
    private function buildNewsletterAnalysisViewModel(
     ...
     */
    private function mapEditorialLinkSummaryRowForResearchView(
        ThoughtLinkSummary $row
    ): array {
    ...
    private function compareEditorialLinkSummariesForDisplay(
        ThoughtLinkSummary $a,
        ThoughtLinkSummary $b
    ): int {
    ...
    private function compareEditorialLinkSummarySectionRank(
        ThoughtLinkSummary $a,
        ThoughtLinkSummary $b
    ): int {
    ...
    private function resolveEmailLinkedResearchUrl(
        ...
        $research = $this->resolveEmailLinkedResearchThought(
            $thought,
            $preloadedImportedEmail,
            $usePreloadedImportedEmail
        );
    ...
    private function resolveEmailLinkedResearchThought(
        ...
        $importedResearchId = $this->normalizeResearchThoughtId(
            $imported?->research_thought_id
        );

        $captured = $this->resolveCapturedInboundEmailForThought($thought);
        $capturedResearchId = $this->normalizeResearchThoughtId(
            $captured?->research_thought_id
        );

        if (
            $importedResearchId !== null &&
            $capturedResearchId !== null &&
            $importedResearchId !== $capturedResearchId
        ) {
            return null;
        }
        ...
        $storedIds = array_values(
            array_unique(
                array_filter([$importedResearchId, $capturedResearchId])
            )
        );
        $storedId = null;
     ...
     */
    private function buildEmailNewsletterResearchStatuses(
        Collection $thoughts
    ): array {
        $statuses = [];

        foreach ($thoughts as $thought) {
            $statuses[$thought->id] = $this->buildEmailNewsletterResearchStatus(
                $thought
            );
        }
     ...
     */
    private function buildEmailNewsletterResearchStatusPresenters(
        Collection $thoughts
    ): array {
    ...
    private function buildEmailResearchPreview(
        ...
        $resolved = $this->resolveEmailLinkedResearchThought(
            $emailThought,
            $preloadedImportedEmail,
            $usePreloadedImportedEmail
        );
    ...
    private function buildResearchPreviewPayloadFromLinkedResearchThought(
        ...
        $documentRoot = $this->resolveResearchDocumentRootForPreview(
            $linkedResearch
        );
        ...
        $sectionHtmlChunks = $sections
            ->take(2)
            ->map(function (Thought $section) use (
                $converter,
                $sectionContext
            ) {
    ...
    private function resolveResearchDocumentRootForPreview(
        Thought $resolvedResearch
    ): ?Thought {
     ...
     */
    private function researchPreviewHasRenderableBody(
        string $rootHtml,
    ...
    private function demoSafeResearchText(
        ?string $value,
        string $context,
        string $boundary
    ): ?string {
     ...
     */
    private function resolveResearchRelatedEmailCard(
        Thought $researchRoot
    ): ?array {
     ...
     */
    private function mergeResearchRootEmailLinkageFields(
        array $metadata,
