...
class BackfillMailAccount implements ShouldQueue
{
    ...
    public function handle(FastmailConnector $connector, EmailImportService $emailImportService): void
    {
        ...
        try {
            $batch = $connector->fetchBackfillBatch($account, [
                'limit' => (int) config('services.mail_sync.backfill_batch_size', 50),
                'mailbox_id' => $account->provider_checkpoint_json['mailbox_id'] ?? null,
            ]);
