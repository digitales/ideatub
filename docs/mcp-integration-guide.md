...
**Dedupe (2026-05):** When content is unchanged (normalized fingerprint, volatile `Last Updated` / `refreshed at` lines stripped by default), upsert returns `deduplicated: true` and the existing `version_id` — no new version row. When content changes, prior `external` versions for that scope are superseded. Optional `strict_content_hash: true` includes volatile lines in the fingerprint. Responses also include `content_fingerprint`, `dedupe_family`, and `superseded_version_id` when applicable. Working-memory `capture_plan` snapshots (tag `working-memory` or `*-working-memory-*` slugs) use the same fingerprint rules; unchanged syncs return `deduplicated: true` without a new Stream card. Nightly `working-memory:dedupe` reconciles recent history; manual backfill: `php artisan working-memory:dedupe --dry-run`.
